...
public class IndexServiceImpl implements IndexService {
  ...
  public Map<String, Object> getUserInfo(String username) {
    ...
    if (null == user) {
      //throw new brilliantException(ResultCodeEnum.FETCH_USERINFO_ERROR);
    }

    //根据用户id获取角色
    List<Role> roleList = roleService.selectRoleByUserId(user.getId());
    List<String> roleNameList = roleList.stream().map(item -> item.getRoleName())
    ...
    //根据用户id获取操作权限值
    List<String> permissionValueList = permissionService.selectPermissionValueByUserId(
        user.getId());
